        ...
        for k in range(len(levelset)):
            ...
            if i not in levelset[k][0]:
                for j in reversed(levelset[k][1]):
                    for w in range(len(candite[i])):
                        if len(j.l) != 0:
                            e = difference_set(j.l,candite[i][w].l)
                            #if len(e) != 0 and len(e) < 5:
                            if (len(sub_set(j.l,candite[i][w].l)) == 0 and len(sub_set(candite[i][w].l,j.l))) != 0:
                                #print(j)
                                newlevelset = list(levelset[k][0])
                                newlevelset.append(i)
                                newitem = iteml(j.name+ ' ' +str(j.sigma)+','+candite[i][w].name+ ' ' +str(candite[i][w].sigma),None,Union_set(candite[i][w].l,j.l),None)
                                #print(newitem)
                                #print(newlevelset)
                                ...
                                preprunelist.sort()
                                # if preprunelist in prunlist and len(preprunelist) > 2:
                                #     continue
                                dependency = ""
                                #lhsSet1 = levelset[k][0]
                                dependency += j.name + ' ' + str(j.sigma)
                                dependency += "->"
                                dependency += candite[i][w].name + ' ' + str(candite[i][w].sigma)
