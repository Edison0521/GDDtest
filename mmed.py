...
'''def differences_set(listA, listB):
    # difference between A & B
    retE = list(set(listB).difference(set(listA)))
    return retE'''
...
def compute_dependencies_and_prune(levelSet, dependency_set, level0Set):
    ...
    for block in levelSet:
        ...
        for level0_block in level0Set:
            if not (level0_block.attributeLabels[0] in lhsLabelSet):
                # print(level0_block.attributeLabels[0], lhsLabelSet)
                for literals in level0_block.literalsSet:
                    # print(literals[0])
                    rhsLiteralSet.append(literals)
        rhsLiteralSet = My_duplicate(rhsLiteralSet)
        # print(My_duplicate(rhsLiteralSet))
        reLiteralsSet = []
        for literals in block.literalsSet:
            # print(type(literals))
            lhsSet = literals[0]
            ...
            for rhs in rhsLiteralSet:
                lhsValueSet = []
                for literal in lhsSet:
                    if literal.name != lhsSet[0].name:
                        # print(literal,rhs)
                        for k in range(len(rhs)):
                            # print(literal.l,rhs[0][k].l)
                            e = difference_set(literal.l, rhs[0][k].l)
                            # print(e)
                            if len(e) == 0:
                                literals_reRank += 1
            if (literals_reRank == len(rhsLiteralSet)):
                reLiteralsSet.append(literals)
                # e = set(literal.l).intersection(rhs[0][k].l)
                # print(e)
                # e = set(literal.l).issubset(rhs[0][k].l)
                # print(e,literal.name,rhs[0][k].name)
            '''if len(e) <= 0:
                                #print(literal.l,rhs[0][k].l)
                                break
                            elif e > 0:
...
if __name__ == "__main__":
    ...
    for attriLabels in L:
        ...
        for i in range(len(candite)):
            # print(candite[i])
            literalsSet.append([candite[i]])
            '''for j in range(len(candite[i])):
                tunple_tempset = []
                tsigmaSet = []
                tunple_tempset.append(candite[i][j].l)
                tsigmaSet.append(candite[i][j].sigma)
            tuplesSet.append(tunple_tempset)
            sigmaSet.append(tsigmaSet)'''
        # print(literalsSet)
        level0Set.append(Block([attriLabels[0]], literalsSet))
        # print('sss',[attriLabels[0]], literalsSet,tuplesSet,sigmaSet)
        # level0Set.append(Blocks([attriLabels[0]], literalsSet,tuplesSet,sigmaSet))
    # print(level0Set)
    lattice.append(level0Set)
    # print(lattice)
    dependency_set = []
    ...
    # print(level0Set)
    # compute_dependencies_and_prune([level0Set[0]],dependency_set,level0Set)
    print(attriNumber)
    for i in range(1, attriNumber + 1):
        ...
        for block1Index, block2Index in combinations([j for j in range(len(lattice[i - 1]))], 2):
            ...
            print(block1.attributeLabels, block2)