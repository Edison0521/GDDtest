...
def combinetitle(title,df):
    ...
    for liter in title:
        e = liter.split('.')
        m = e[0].split(":")
        x = GDD.sattr(m[0].replace("(", ''), m[1].replace(")", ""), e[1].rstrip(),liter,re.sub('[^a-zA-Z]+', '', e[1]))
        titlelist.append(x)
            ...
            for val in g[item]:
                if len(g[item][val]) > 2:
                    ...
                    for var1 in c:
                        ...
                        iteml = GDD.iteml(s,var1[0].name,var1[0].attribute,0)
                        if iteml not in sb:
                            sb.append(iteml)
...
def distribution_combination(lattice, i, block1Index, block2Index, levelSet):
    block1 = lattice[i - 1][block1Index]
    block2 = lattice[i - 1][block2Index]
    #print(block1,block2)
    prefixSame = True
    if (len(block1.attributeLabels) > 1):
        #print(block1.attributeLabels)
        # only the set which the prefixs are same can be combined to generate a new set in the next level
        if (not (operator.eq(block1.attributeLabels[0:len(block1.attributeLabels) - 1],
                             block2.attributeLabels[0:len(block2.attributeLabels) - 1]))):
            prefixSame = False
    if (len(block1.literalsSet) != 0 and len(block2.literalsSet) != 0 and prefixSame):
        literalsSet = []
        attributeLabels = []
        combineFirstOrder = 1
        if block1.attributeLabels[len(block1.attributeLabels) - 1] < block2.attributeLabels[
            len(block2.attributeLabels) - 1]:
...
def virtual_combination(c):
    distribution_combination(c[0], c[1], c[2], c[3], c[4])
        ...
        for block1Index, block2Index in combinations([j for j in range(len(lattice[i - 1]))], 2):
            ...
            if (len(block1.attributeLabels)>1):
                ...
                if (not(operator.eq(block1.attributeLabels[0:len(block1.attributeLabels)-1],block2.attributeLabels[0:len(block2.attributeLabels)-1]))):
                    prefixSame = False
            if (len(block1.literalsSet)!=0 and len(block2.literalsSet)!=0 and prefixSame):
                #print(lattice, i, block1Index, block2Index, levelSet)
                parameterList.append([lattice, i, block1Index, block2Index, levelSet])
        pool.map(virtual_combination, parameterList)
        #level += 1
        print("levelSet", len(levelSet))
        end_time = time.time()
        print('one_level_combination_time_cost', end_time - start_time, "s")
        start_time = time.time()
        levelSet = list(levelSet)
        ...
        for block2 in levelSet2:
            ...
            if (set(attributeLabels_1).issubset(set(attributeLabels_2))):
                ...
                for reliterals in reLiteralsSet:
                    # prune
                    if reliterals in block2.literalsSet:
...
if __name__ == "__main__":
    ...
    #for gi in g:
        ...
        latticee.append(level0Set)
        dependency_set = []
        mainMethod(latticee, level0Set, dependency_set, literals)
