...
def relationbolck(item):
    if 'id' not in item:
        ...
        distance = 0
        distancelist = splitdistance(distance, 1)
        nxms = []
        for dis in distancelist:
            l1 = []
            for val in cleanlist:
...
def combinetitle(title,df):
        ...
        if str(item) != 'id':
            for val in g[item]:
                if len(g[item][val]) > 2:
                    ...
                    #print(c,g[item][val][0].name,g[item][val][0].attribute)
                    pc.append(GDD_new.node_s(g[item][val][0].name,g[item][val][0].attribute,c))
                elif len(g[item][val]) <= 2:
                    pp.append(g[item][val][0].lists)
                    #r = relationbolck(g[item][val][0].lists)
                    #constblock.append(r)
        elif str(item) == 'id':
            for val in g[item]:
                pi.append(val)
    ...
    with multiprocessing.Pool(processes=8) as pool:
        # 使用3个进程并行处理items
        cresults = pool.map(relationbolck, pp)
    with multiprocessing.Pool(processes=8) as pool:
        # 使用3个进程并行处理items
        presults = pool.map(ppblock, pc)

    patternblock = presults
    constblock = cresults
...
def ppblock(var1):
    sb = []
    #print(var1)
    for i in range(len(var1.attribute)):
        #print(str(re.sub('[^a-zA-Z]+', '', str(var1.id)) + '.' + str(var1.name) + '=' + str(item)))
        s = str(re.sub('[^a-zA-Z]+', '', str(var1.id)) + '.' + str(var1.name) + '=' + str(var1.attribute[i][0].id))
        iteml = GDD_new.iteml(s, var1.attribute[i][0].name, var1.attribute[i][0].attribute, 0)
        #print(iteml)
        if iteml not in sb:
            sb.append(iteml)
    return sb
