...
def generLiterals(readFile):
        ...
        if attribute != "id":
            for value in partiton.keys():
                ...
                if value == value and value != "N":
                    ...
                    literals.append(
                        Literal(temp1, variable, attribute, value, False, None, None, equalClass, entityType))
                    temp1 += 1
    # print(literals)
    ...
    for situation in situations:
        ...
        se = pd.Series(df["(" + entityType + ":" + vari1 + ")" + "." + attri1] == df[
            "(" + entityType + ":" + vari2 + ")" + "." + attri2])
        equalClass = tuple(se[se].index)
        literals.append(
            Literal(temp1, "None", attri1, None, True, (vari1, vari2), (attri1, attri2), equalClass, entityType))
        temp1 += 1
...
def mainMethod(readFile, dependency_set, information):
    literals, attriNumber = generLiterals(readFile)
                    ...
                    for literals2 in block2.literalsSet:
                        ...
                        if (operator.eq(literals1Id, literals2Id)):
                            ...
                            if literals1[length - 1].attributeLabel < literals2[length - 1].attributeLabel:
                                literalSet.append(literals1[length - 1]);
                                literalSet.append(literals2[length - 1])
                            else:
                                literalSet.append(literals2[length - 1]);
                                literalSet.append(literals1[length - 1])
...
if __name__ == "__main__":
    ...
    dependency_set = []
    information = []
    mainMethod(readFile, dependency_set, information)
